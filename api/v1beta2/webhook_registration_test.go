...
func TestWebhookRegistrationsServeAdmissionReviews(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "OCICluster",
			object:   &OCICluster{TypeMeta: metav1.TypeMeta{APIVersion: GroupVersion.String(), Kind: "OCICluster"}},
			...
			setup: func(mgr ctrl.Manager) error {
				return (&OCICluster{}).SetupWebhookWithManager(mgr)
			},
		},
		{
			name:     "OCIMachineTemplate",
			object:   &OCIMachineTemplate{TypeMeta: metav1.TypeMeta{APIVersion: GroupVersion.String(), Kind: "OCIMachineTemplate"}},
			resource: "ocimachinetemplates",
			paths: []string{
				"/validate-infrastructure-cluster-x-k8s-io-v1beta2-ocimachinetemplate",
			},
			setup: func(mgr ctrl.Manager) error {
				return (&OCIMachineTemplate{}).SetupWebhookWithManager(mgr)
			},
		},
		{
			name:     "OCIManagedCluster",
			object:   &OCIManagedCluster{TypeMeta: metav1.TypeMeta{APIVersion: GroupVersion.String(), Kind: "OCIManagedCluster"}},
			resource: "ocimanagedclusters",
			paths: []string{
				"/mutate-infrastructure-cluster-x-k8s-io-v1beta2-ocimanagedcluster",
				"/validate-infrastructure-cluster-x-k8s-io-v1beta2-ocimanagedcluster",
			},
			setup: func(mgr ctrl.Manager) error {
				return (&OCIManagedCluster{}).SetupWebhookWithManager(mgr)
			},
		},
		{
			name:     "OCIManagedControlPlane",
			object:   &OCIManagedControlPlane{TypeMeta: metav1.TypeMeta{APIVersion: GroupVersion.String(), Kind: "OCIManagedControlPlane"}},
			resource: "ocimanagedcontrolplanes",
			paths: []string{
				"/mutate-infrastructure-cluster-x-k8s-io-v1beta2-ocimanagedcontrolplane",
				"/validate-infrastructure-cluster-x-k8s-io-v1beta2-ocimanagedcontrolplane",
			},
			setup: func(mgr ctrl.Manager) error {
				return (&OCIManagedControlPlane{}).SetupWebhookWithManager(mgr)
			},
		},
	}
